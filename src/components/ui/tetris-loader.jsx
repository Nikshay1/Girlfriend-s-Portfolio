...
export default function TetrisLoading({
            ...
            <div key={rowIndex} className="flex">
                {row.map((cell, colIndex) => (
                    <div
                        ...
                        className={`${config.cellSize} border border-white/10 transition-all duration-100 ${cell.filled
                                ? `${cell.color} scale-100 shadow-[0_0_10px_rgba(255,255,255,0.5)]`
    ...
    return (
        <div className="flex flex-col items-center justify-center">
            <div className="mb-6">
                <div className={`rounded-lg bg-black/40 backdrop-blur-md border border-white/10 ${config.padding} transition-colors shadow-2xl`}>
                    {renderGrid()}
                </div>
            </div>
            ...
            {showLoadingText && (
                <div className="text-center animate-pulse">
                    <p className="text-white/70 font-medium tracking-widest text-sm uppercase">{loadingText}</p>
                </div>
            )}
        </div>
    )
